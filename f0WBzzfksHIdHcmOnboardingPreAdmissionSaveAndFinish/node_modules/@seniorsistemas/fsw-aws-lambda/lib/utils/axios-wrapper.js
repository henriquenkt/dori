...
class AxiosWrapper {
    ...
    platformPost(event, uri, data, config) {
        return this.post(`${event.platformUrl}${uri}`, data, this._configToken(event, config));
    }

    platformPut(event, uri, data, config) {
        return this.put(`${event.platformUrl}${uri}`, data, this._configToken(event, config));
    }

    platformPatch(event, uri, data, config) {
        return this.patch(`${event.platformUrl}${uri}`, data, this._configToken(event, config));
    }

    platformGetWithLambdaToken(event, uri, config) {
        return this.get(`${event.platformUrl}${uri}`, this._configLambdaToken(event, config));
    }

    platformDeleteWithLambdaToken(event, uri, config) {
        return this.delete(`${event.platformUrl}${uri}`, this._configLambdaToken(event, config));
    }

    platformHeadWithLambdaToken(event, uri, config) {
        return this.head(`${event.platformUrl}${uri}`, this._configLambdaToken(event, config));
    }

    platformPostWithLambdaToken(event, uri, data, config) {
        return this.post(`${event.platformUrl}${uri}`, data, this._configLambdaToken(event, config));
    }

    platformPutWithLambdaToken(event, uri, data, config) {
        return this.put(`${event.platformUrl}${uri}`, data, this._configLambdaToken(event, config));
    }

    platformPatchWithLambdaToken(event, uri, data, config) {
        return this.patch(`${event.platformUrl}${uri}`, data, this._configLambdaToken(event, config));
    }
    ...
    _configLambdaToken(event, config) {
        config = config || {};
        ...
        if (!config.headers['Authorization'] && event.lambdaToken) {
            config.headers['Authorization'] = event.lambdaToken;
        }
